    ...
    def forward(self, graph, G, h, pretrain_mode, mode, block = None, only_relation = None):
        with graph.local_scope():
            ...
            if only_relation is not None:
                if only_relation[0][1] == 'indication':
                    etypes_train = [('drug', 'indication', 'disease'),
                                    ('disease', 'rev_indication', 'drug')]
                elif only_relation[0][1] == 'contraindication':
                    etypes_train = [('drug', 'contraindication', 'disease'), 
                                   ('disease', 'rev_contraindication', 'drug')]
                elif only_relation[0][1] == 'drug_protein':
            ...
            if pretrain_mode:
                ...
                for etype in etypes_train:
                    if self.proto:
                        ...
                        else: ## if fine tuning just infers the head node type
                            head = only_relation[0][0]
                            ## for smiplicity use this same h_disease dict, even though the nodes dont have to be dissease
                            if src == head:
                                h_disease['disease_query'] = src_h
                                h_disease['disease_key'] = src_h_keys
                                h_disease['disease_query_id'] = src_rel_idx
                        ...
                        if self.sim_measure in ['protein_profile', 'all_nodes_profile', 'protein_random_walk', 'bert', 'profile+bert', 'all_nodes_profile_more']:
                            ...
                            try:
                                ...
                                out = torch.mul(embed, coef.unsqueeze(dim = 2)).sum(dim = 1)
...
class HeteroRGCN(nn.Module):
    ...
    def forward(self, G, neg_G, eval_pos_G = None, return_h = False, return_att = False, mode = 'train', pretrain_mode = False,  only_relation = None):
        with G.local_scope():
            ...
            if eval_pos_G is not None:
                # eval mode
                scores, out_pos = self.pred(eval_pos_G, G, h, pretrain_mode, mode = mode + '_pos', only_relation = only_relation)
                scores_neg, out_neg = self.pred(neg_G, G, h, pretrain_mode, mode = mode + '_neg', only_relation = only_relation)
                return scores, scores_neg, out_pos, out_neg
            else:
                scores, out_pos = self.pred(G, G, h, pretrain_mode, mode = mode + '_pos', only_relation = only_relation)
                scores_neg, out_neg = self.pred(neg_G, G, h, pretrain_mode, mode = mode + '_neg', only_relation = only_relation)
                return scores, scores_neg, out_pos, out_neg

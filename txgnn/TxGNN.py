...
class TxGNN:
    ...
    def __init__(self, data,
        ...
        # np.unique(df[self.df["x_type"]=='drug']["y_type"])
        ...
        ## buz change removed off label added drug to proten
        #self.disease_rel_types = ['rev_contraindication', 'rev_indication', 'rev_off-label use']
        self.disease_rel_types = ['rev_contraindication', 'rev_indication', 'rev_drug_protein']
        
        ## buz changes muted off label added drug to protien 
    ...
    def model_initialize(self, n_hid = 128, 
        ...
        ## missing rev_anatomy_protein_absent, anatomy_protein_absent
    ...
    ## buz change since having issues with cuda drivers 
        try:
    ...
    def pretrain(self, n_epoch = 1, learning_rate = 1e-3, batch_size = 1024, train_print_per_n = 20, sweep_wandb = None):
        ...
        ## buz change this is a version issue 
        try:
        ...
        for epoch in range(n_epoch):
            ...
            ## buz change since having issues with cuda
                try:
                    loss = F.binary_cross_entropy(scores, torch.Tensor(labels).float().to(self.device))
                except:
                    loss = F.binary_cross_entropy(scores, torch.Tensor(labels).float())
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
    ...
    def finetune(self, n_epoch = 500, 
        ...
        if finetune_on is not None:
            print("Finetuning on {0} relation".format(finetune_on[1]))
        else:
            print("Finetuning on all disease relations")
        best_val_acc = 0
        ...
        for epoch in range(n_epoch):
            ...
            if finetune_on is None:
                ...
                ## buz change define only_relation and finetune_relations 
                only_relation = [finetune_on]
                finetune_relations = only_relation + [(triplet[2] , 'rev_'+triplet[1] , triplet[0]) for triplet in only_relation]
                pred_score_pos, pred_score_neg, pos_score, neg_score = self.model(self.G, negative_graph, pretrain_mode = False, mode = 'train', only_relation = only_relation)
                pos_score = torch.cat([pred_score_pos[i] for i in finetune_relations])
                neg_score = torch.cat([pred_score_neg[i] for i in finetune_relations])
            ...
            if (epoch) % valid_per_n == 0:
                # validation tracking...
                print('Validation.....')
                (auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc), loss = evaluate_fb(self.model, self.g_valid_pos, self.g_valid_neg, self.G, self.dd_etypes, self.device, mode = 'valid', finetune_on=finetune_on)
        ...
        (auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc), loss, pred_pos, pred_neg = evaluate_fb(self.best_model, self.g_test_pos, self.g_test_neg, self.G, self.dd_etypes, self.device, True, mode = 'test', finetune_on=finetune_on)
    ...
    def predict(self, df):
        ...
        for etype in g.canonical_etypes:
            ...
            try:
                ...
                src = torch.Tensor(df_temp.x_idx.values).to(dtype = torch.int64)
                dst = torch.Tensor(df_temp.y_idx.values).to(dtype = torch.int64)
    ...
    def retrieve_sim_diseases(self, relation, k = 5, path = None):
        ...
        if relation not in ['indication', 'contraindication', 'drug_protein']:
            raise ValueError("Please select the following three relations: 'indication', 'contraindication', 'drug_protein' !")
        ...
        ## buz change removed off-label added drug_protein
    ...
    def train_graphmask(self, relation = 'indication',
        ...
        # buz change added drug-protien removed off-label
        # if relation not in ['indication', 'contraindication', 'off-label']:
        ...
        if relation == 'indication':
            ...
            etypes_train = [('drug', 'contraindication', 'disease'), 
                           ('disease', 'rev_contraindication', 'drug')]
        # buz change - removed off label added drug-protein
        # elif relation == 'off-label':
        #     etypes_train = [('drug', 'off-label use', 'disease'),
        #                    ('disease', 'rev_off-label use', 'drug')]
        elif relation == "drug_protein":
            etypes_train = [('drug', 'drug_protein', 'gene/protein'),
                            ('drug', 'rev_drug_protein', 'gene/protein')]
        else:

...
def random_fold(df, fold_seed, frac):
    ...
    for i in df.relation.unique():
        ...
        df_train = pd.concat([df_valid, val], ignore_index=True)
...
def evaluate_graph_construct(df_valid, g, neg_sampler, k, device):
    ...
    for etype in g.canonical_etypes:
        ...
        try:
            ...
            try:
                src = torch.Tensor(df_temp.x_idx.values).to(dtype = torch.int64).to(device).to(dtype = torch.int64)
                dst = torch.Tensor(df_temp.y_idx.values).to(dtype = torch.int64).to(device).to(dtype = torch.int64)
            except:
...
def evaluate_fb(model, g_pos, g_neg, G, dd_etypes, device, return_embed = False,finetune_on=None, mode = 'valid'):
    model.eval()
    ...
    if finetune_on is None:
        ...
        pred_score_pos, pred_score_neg, pos_score, neg_score = model(G, g_neg,g_pos, pretrain_mode = False, mode = mode, only_relation = only_relation)
...
def create_dgl_graph(df_train, df):
    ...
    # df[(df["x_type"] == "disease")&(df["y_type"] == "disease")]
    DGL_input = {}
    # import ipdb; ipdb.set_trace()

...
class TxData:
    ...
    def retrieve_id_mapping(self):
        ...
        ## buz change added idx to protein 
        idx2id_protein = dict(df[df.x_type == 'gene/protein'][['x_idx', 'x_id']].drop_duplicates().values)
        idx2id_protein.update(dict(df[df.y_type == 'gene/protein'][['y_idx', 'y_id']].drop_duplicates().values))
        ...
        ## buz change updated idx to protien 
        id2name_protein = dict(df_[df_.x_type == 'gene/protein'][['x_id', 'x_name']].drop_duplicates().values)
        id2name_protein.update(dict(df_[df_.y_type == 'gene/protein'][['y_id', 'y_name']].drop_duplicates().values))

        ## buz change updated what is returned 
        return {'id2name_drug': id2name_drug,
                'id2name_disease': id2name_disease,
                'idx2id_disease': idx2id_disease,
                'idx2id_drug': idx2id_drug,
                'id2name_protein': id2name_protein,
                'idx2id_protein': idx2id_protein
               }